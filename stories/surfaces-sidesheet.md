...
> **Package:** `@equinor/eds-core-react` — `import { SideSheet } from '@equinor/eds-core-react'`

| Name | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| `title` | `string` | No |  | Title for Side Sheet |
| `variant` | `"small" "medium" "large" "xlarge"` | No | medium | Variant controls width of Side Sheet |
| `onClose` | `(Event: any) => void` | No |  | OnClick function (close) |
| `open` | `boolean` | Yes |  | Open / close Side Sheet |
| `width` | `string` | No |  | Override width of Side Sheet |
...
() => {
  ...
  return <div ref={containerRef} style={{
    ...
    isolation: 'isolate'
      ...
      <Button variant="outlined" onClick={() => setToggle(!toggle)} style={{
      float: 'right'
    }}>
        Filters
      </Button>

      <SideSheet open={toggle} onClose={() => setToggle(!toggle)} width={`${width}px`}>
        ...
        padding: '0 8px'
          ...
          <UnstyledList>
            <li>
              <Checkbox label="Tables" name="multiple" value="first" defaultChecked />
            </li>
            <li>
...
() => {
  ...
  return <>
      ...
      <SideSheet open={toggle} onClose={() => setToggle(!toggle)} style={{
      height: '90%'
    }}>
        <div style={{
        padding: '0 5px'
          ...
          <Slider min={getUnixTime('1960')} max={getUnixTime('2020')} aria-labelledby="date-range-slider" value={[getUnixTime('1980'), getUnixTime('2000')]} outputFunction={outputFunction} />
        </div>
      </SideSheet>
    </>;
}
...
() => {
  ...
  return <>
      ...
      <Scrim open={open} onClose={() => setOpen(!open)} isDismissable style={{
      position: 'absolute'
    }}>
        <SideSheet open={open} onClose={() => setOpen(!open)} style={{
        overflowY: 'hidden'
      }}>
          <SheetWrapper>
