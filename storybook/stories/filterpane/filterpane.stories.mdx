...
the filter provider is the only give component in this hierachy. It provides the context that is used in all the other components.
...
The bar will handle `children` if added. They will flow after the buttons/chips.
...
If `useFilterSelector` is not used. All registered filteres will be shown at all times.
...
`filter` takes in an Filter object that has all the necessery setup (see filter settings).
