...
- [PeopleViewer](https://equinor.github.io/fusion-react-components/?path=/story/person-people-peopleviewer--docs) - Component to display a list or table of selected people.
...
The standard `PersonProvider` is provided by the Fusion portal or dev-server from `fusion-framework-cli`.
...
<PeoplePicker
  ...
  noResultSubtitle="Try adjusting your search criteria." // Custom subtitle to display when no results are found.
  display="list|table"
  displayToggle={true} // Whether to show the display mode selector. Defaults to true.
...
#### Resolving with IDs, as a controlled component
...
Add `multiple={false}` to the props of `PeoplePicker` to force single selection.
...
#### ResolveIds, as a controlled component

<Canvas of={ peopleViewerStories.resolveIds } meta={peopleViewerStories} />

#### People provided as props as a controlled component
