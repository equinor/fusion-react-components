...
/**
 * Handles highlightedItem. Sets highlighted item when an item is selected.
 *
...
const useHightLightedItem = <T extends HangingGardenColumnIndex>() => {
  ...
  const renderHighlightedItem = useCallback(() => {
    if (!highlightedItem) {
      const renderedHighlightedItem = getTextureFromCache('items', HIGHLIGHTED_ITEM_KEY) as PIXI.Container;
    ...
    const column = (columns as HangingGardenColumn<T>[]).find((column) =>
      column.data.find((item: T) => item[itemKeyProp as keyof T] === (highlightedItem as T)[itemKeyProp as keyof T])
    );
