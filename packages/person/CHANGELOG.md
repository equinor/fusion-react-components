...
- bead18c: Update `@equinor/fusion-wc-people` and `@equinor/fusion-wc-person` dependencies.
...
- Updated dependencies [c2a474d]
- Updated dependencies [fd142b9]
- Updated dependencies [9094cbb]
  - @equinor/fusion-react-utils@3.0.3
...
- 89ceb65: Security patch release. No functional changes.
...
- c0fe19f: Updated @equinor/fusion-wc-people from 2.0.0 to 2.0.1
...
- f3e9518: - Add PeoplePicker, PersonPicker, and PeopleViewer components to the person package, including React event helpers and updated exports for people selection flows.
  - Update properties for `PersonAvatar`, `PersonCard`, `PersonCell` and `PersonListItem` to include `resolveId` which can be either an azureId or upn. This also deprecates the `azureId` and `upn` property which are marked for removal in next major release
