...
import { context, PowerBIEmbedComponent, PowerBIEmbedEvents } from '../../context';
...
export const PowerBIReportView: FC<PowerBIComponentProps> = ({ config }: PowerBIComponentProps) => {
  ...
  const eventHandlers = useMemo(
    () =>
      Object.values(PowerBIEmbedEvents).reduce(
        ...
        new Map<unknown, EventHandler>()
      ),
    [event$]
  );

  const getEmbeddedComponent = (value: PowerBIEmbedComponent) => {
    if (component) {
  ...
  if (embedConfig) {
    return (
      ...
      // @ts-ignore
      <PowerBIEmbed {...{ embedConfig, eventHandlers, getEmbeddedComponent, cssClassName, service }}></PowerBIEmbed>
    );
  }
  return null;
};
