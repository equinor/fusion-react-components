...
import { filter, switchMap, map, catchError, takeUntil, delay, withLatestFrom, throttleTime } from 'rxjs/operators';
...
export const accessToken = <S extends { id: string; token: AccessToken }>(
  ...
  const request$ = action$.pipe(
    ...
    switchMap((action) =>
      from(clients.acquireAccessToken(action.payload.reportId)).pipe(
        map((res) => actions.fetchAccessToken.success(res)),
  ...
  const acquired$ = action$.pipe(
    ...
    switchMap((action) => {
      ...
      return of(actions.refreshAccessToken({ reason: 'expired since acquired' })).pipe(delay(expires));
    })
  );
