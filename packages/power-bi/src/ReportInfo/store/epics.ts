...
export const fetchReport = <S = any>(action$: Observable<Actions>, _: StatefulObserver<S>, { clients }: Dependencies) =>
  action$.pipe(
    filter(isActionOf(actions.fetchReport.request)),
    switchMap((action) =>
      from(clients.report.getReportAsync(action.payload)).pipe(
        map((res) => actions.fetchReport.success(res.data)),
        catchError((error) => of(actions.fetchReport.failure({ action, error }))),
        takeUntil(action$.pipe(filter(isActionOf(actions.fetchReport.cancel))))
      )
    )
  );
...
export const fetchReportAccessDescription = <S = any>(
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(clients.report.getAccessDescription(action.payload)).pipe(
        ...
        takeUntil(action$.pipe(filter(isActionOf(actions.fetchReportAccessDescription.cancel))))
      )
    )
  );
...
export const fetchReportDescription = <S = any>(
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(clients.report.getDescription(action.payload)).pipe(
        ...
        takeUntil(action$.pipe(filter(isActionOf(actions.fetchReportDescription.cancel))))
      )
    )
  );
...
export const fetchReportRequirements = <S = any>(
  ...
  action$.pipe(
    ...
    switchMap((action) =>
      from(clients.report.getRlsRequirements(action.payload)).pipe(
        ...
        takeUntil(action$.pipe(filter(isActionOf(actions.fetchReportRequirements.cancel))))
      )
    )
  );

export const initialize = (action$: Observable<Actions>, state$: StatefulObserver<State>, deps: Dependencies) => {
  ...
  return action$.pipe(
    ...
    switchMap((action) => concat(request$(action.payload), of(actions.initialize.success())).pipe(takeUntil(abort$)))
  );
};
