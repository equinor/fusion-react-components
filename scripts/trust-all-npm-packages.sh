...
# 1. login to npm if not already done (interactive)
# 2. run this script to trust all workspace packages for the release workflow
#   - copy first stop eg. `npm trust list @equinor/<package-name>`
#   - stop script and run manually approve EOTP token access for 5 minutes
#   - run script again to add trust entries


set -euo pipefail

ORG="${ORG:-equinor}"
REPO="${REPO:-fusion-react-components}"
